...
static std::string get_ready_events_string(std::uint32_t events)
{
...
Timestamp EPoll::poll(std::chrono::milliseconds ms, std::vector<Channel*>& ready_channels)
{
    ...
    if (ret > 0)
    {
        ...
        for (int i : std::views::iota(0, ret))
        {
            ...
            LOG_INFO("channel of fd:{} has ready events:{}", channel->get_fd(), get_ready_events_string(m_epoll_events[i].events));
